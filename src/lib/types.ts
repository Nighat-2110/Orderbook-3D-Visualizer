  ...
  data.forEach((val, i) => {
    if (val > threshold) {
      if (!currentZone) {
        ...
        currentZone.average =
          (currentZone.average * currentZone.size + val) /
          (currentZone.size + 1);
        currentZone.size += 1;
      }
    } else if (currentZone) {
...
export function getPressureZones(
  ...
  const pressureZonesBids = detectPressureZones(bidVolumes, thresholdMultiplier).map(
    (z) => z.startIndex
  );
  const pressureZonesAsks = detectPressureZones(askVolumes, thresholdMultiplier).map(
    (z) => z.startIndex
  );
